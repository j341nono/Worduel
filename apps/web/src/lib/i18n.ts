...
const ja: Dict = {
  ...
  home: {
    description:
      "60秒の1対1の単語デュエル。試合開始時に同じ3文字単語のセットが両者へ配られます。より多く、より速く解いて、タイマー終了時にスコアが高い方が勝ち！",
    startCpu: "CPUと対戦する",
    ...
    rule2Title: "共通問題",
    rule2Body: "現在の対戦ルールでは出題操作はありません。両者が同じ問題セットを同時に解きます。",
    rule3Title: "パズル",
    rule3Body: "5スロット固定。1回の入力で全ての未解決パズルに同時挑戦。何度でも予想OK。",
    rulesTitle: "ルール詳細",
  },
  rules: {
    section1Title: "勝利条件",
    ...
    section2Title: "共通問題",
    section2Body:
      "以前のルールではトークンを使って相手に出題していましたが、現在は試合開始時にサーバが5つの共通単語を決めます。自分も相手も同じ答えセットに挑戦するため、勝負は解答速度と正解数で決まります。",
    section3Title: "解答のしくみ",
    section3Body:
      "画面には5つのパズルスロットが常に表示されます。あなたが入力する3文字は、進行中のすべてのパズルに同時に適用されます。正解したスロットはロックされ、それ以降はそのままの状態で残ります（答えも表示されます）。1パズルあたりの予想回数は無制限。文字ごとに次のフィードバックが返ります：『正解(緑)』＝文字も位置も合っている、『近い(黄)』＝文字は使われているが位置が違う、『はずれ(灰)』＝その文字は答えに含まれない。",
    section4Title: "スコア計算",
    section4Body:
      "正解した側：+10点（基本）／+5点（1回目正解）・+3点（2回目）・+1点（3回目）／速さボーナス＝max(0, 15 − 解答秒数)。4回目以降の予想で正解した場合はボーナスなしで +10点のみ。未解決のスロットには得点が入りません。",
    section5Title: "戦略のヒント",
    section5Body:
      "難しいパズルでも、解いている過程で得た色情報を他のパズルにも使えるので、複数同時並行で進めると効率的です。相手と問題が同じなので、迷った時間はそのままスコア差になりやすいです。",
  },
  cpu: {
    title: "CPUと対戦",
    description: "CPU相手に練習できます。共通問題・スコア・タイミングは本番ルールと同じです。",
    start: "対戦開始",
    difficultyEasy: "やさしい",
  ...
  hud: {
    ...
    phaseFinished: "終了",
    offline: "切断",
    solved: "正解 {{solved}}/{{total}}",
  },
  puzzle: {
    incoming: "共通パズル",
    noActive: "現在パズルはありません",
    waitForOpponent: "対戦開始を待ちましょう。",
    guessBtn: "予想する",
...
const en: Dict = {
  ...
  home: {
    description:
      "A 60-second 1-on-1 word duel. Both players receive the same set of 3-letter words at match start. Solve more of them, solve them faster, and finish with the higher score.",
    startCpu: "Start CPU Battle",
    createRoom: "Create Online Room",
    joinRoom: "Join Room",
    rule1Title: "Match length",
    rule1Body: "60 seconds, sudden-stop scoring.",
    rule2Title: "Shared words",
    rule2Body: "The current rules do not use manual sending. Both players solve the same shared set.",
    rule3Title: "Puzzles",
    rule3Body: "Five fixed slots. Each guess hits every unsolved puzzle at once — unlimited attempts.",
  ...
  rules: {
    ...
    section2Title: "Shared words",
    section2Body:
      "Older builds used tokens to send puzzles to the opponent. The current rules choose five shared words on the server when the match starts. Both players solve the same answer set, so the duel is decided by solves and speed.",
    section3Title: "Solving",
    section3Body:
      "Five puzzle slots are visible at all times. The 3-letter word you type is applied simultaneously to EVERY active puzzle — each gets its own color feedback. Solved slots get locked in place (the answer is revealed). There is no per-puzzle guess limit, so keep guessing until the slot is solved or the match ends.",
    section4Title: "Scoring",
    section4Body:
      "Solver: +10 base, +5/+3/+1 for guess #1/#2/#3, +max(0, 15 − seconds) speed bonus. Solves on guess 4 or later still score +10 with no bonus. Unsolved slots do not score.",
    section5Title: "Strategy tips",
    section5Body:
      "Letters you learn from one puzzle still inform the others, so working multiple puzzles in parallel is efficient. Because the opponent has the same words, hesitation turns quickly into a score gap.",
  },
  cpu: {
    title: "CPU Battle",
    description: "Practice against the CPU. Shared words, scoring and timing match online play.",
    start: "Start match",
    difficultyEasy: "easy",
  ...
  puzzle: {
    incoming: "Shared puzzles",
    noActive: "No active puzzle.",
    waitForOpponent: "Wait for the match to start.",
    guessBtn: "Guess",
  ...
  feed: {
    ...
    opponentPuzzle: "Opponent puzzle",
    sharedSolved: "— solved",
    sharedMissed: "— unsolved",
